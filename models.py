...
def init_db():
    ...
    try:
        ...
        conn.execute("PRAGMA foreign_keys = ON")
    # Migration: fix settings table FK if it references users_old
    try:
        fk_list = conn.execute("PRAGMA foreign_key_list(settings)").fetchall()
        if any(row[2] == "users_old" for row in fk_list):
            conn.execute("PRAGMA foreign_keys = OFF")
            conn.execute("""CREATE TABLE settings_new (
                user_id INTEGER NOT NULL REFERENCES users(id),
                key TEXT NOT NULL,
                value TEXT NOT NULL,
                PRIMARY KEY (user_id, key)
            )""")
            conn.execute("INSERT INTO settings_new SELECT user_id, key, value FROM settings")
